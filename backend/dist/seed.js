...
async function seedDemoData() {
    const existing = await (0, db_1.query)("SELECT COUNT(*)::text AS count FROM clients WHERE is_demo = true");
    ...
    for (const client of clients) {
        const inserted = await (0, db_1.query)("INSERT INTO clients (name, phone, address, notes, interactions, is_demo) VALUES ($1,$2,$3,$4,$5,true) RETURNING id", client);
    ...
    for (const project of projects) {
        const inserted = await (0, db_1.query)("INSERT INTO projects (name, type, client_id, duration_days, expected_cost, actual_cost, start_date, end_date, status, is_demo) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,true) RETURNING id", project);
        projectIds.push(inserted.rows[0].id);
    }
    const stageNames = ["كهرباء", "سباكة", "محارة", "دهانات", "تشطيبات نهائية"];
    for (const projectId of projectIds) {
        for (const stageName of stageNames) {
            await (0, db_1.query)("INSERT INTO project_stages (project_id, name, progress, is_demo) VALUES ($1,$2,$3,true)", [projectId, stageName, Math.floor(Math.random() * 55) + 45]);
        }
    }
    const contractors = [
    ...
    for (const person of [...contractors, ...workers]) {
        await (0, db_1.query)("INSERT INTO workforce (name, worker_type, specialization, rating, payment_due, phone, is_demo) VALUES ($1,$2,$3,$4,$5,$6,true)", person);
    }
    const inventory = [
    ...
    for (const item of inventory) {
        const inserted = await (0, db_1.query)("INSERT INTO inventory_items (name, unit, quantity, min_quantity, supplier, is_demo) VALUES ($1,$2,$3,$4,$5,true) RETURNING id", item);
        await (0, db_1.query)("INSERT INTO inventory_movements (item_id, movement_type, quantity, note, is_demo) VALUES ($1,'إضافة',$2,$3,true)", [inserted.rows[0].id, item[2], "رصيد افتتاحي"]);
    }
    const financial = [
    ...
    for (const row of financial) {
        await (0, db_1.query)("INSERT INTO financial_records (record_type, project_id, description, amount, record_date, is_demo) VALUES ($1,$2,$3,$4,$5,true)", row);
    }
    await (0, db_1.query)("INSERT INTO invoices (invoice_no, client_id, project_id, total, paid, status, due_date, is_demo) VALUES ($1,$2,$3,$4,$5,$6,$7,true),($8,$9,$10,$11,$12,$13,$14,true),($15,$16,$17,$18,$19,$20,$21,true)", [
    ...
    for (const property of properties) {
        const inserted = await (0, db_1.query)("INSERT INTO properties (name, property_type, status, location, price, is_demo) VALUES ($1,$2,$3,$4,$5,true) RETURNING id", property);
        propertyIds.push(inserted.rows[0].id);
    }
    await (0, db_1.query)("INSERT INTO installments (property_id, customer_name, amount, due_date, is_paid, is_demo) VALUES ($1,$2,$3,$4,$5,true),($6,$7,$8,$9,$10,true),($11,$12,$13,$14,$15,true)", [
...
async function clearDemoData() {
    await (0, db_1.query)("DELETE FROM installments WHERE is_demo = true");
    await (0, db_1.query)("DELETE FROM properties WHERE is_demo = true");
    await (0, db_1.query)("DELETE FROM invoices WHERE is_demo = true");
    await (0, db_1.query)("DELETE FROM financial_records WHERE is_demo = true");
    await (0, db_1.query)("DELETE FROM inventory_movements WHERE is_demo = true");
    await (0, db_1.query)("DELETE FROM inventory_items WHERE is_demo = true");
    await (0, db_1.query)("DELETE FROM project_assignments WHERE is_demo = true");
    await (0, db_1.query)("DELETE FROM workforce WHERE is_demo = true");
    await (0, db_1.query)("DELETE FROM project_stages WHERE is_demo = true");
    await (0, db_1.query)("DELETE FROM projects WHERE is_demo = true");
    await (0, db_1.query)("DELETE FROM clients WHERE is_demo = true");
}
async function ensureDefaultUsers() {
    const adminPassword = await bcryptjs_1.default.hash("Admin@123", 10);
    const accountantPassword = await bcryptjs_1.default.hash("Account@123", 10);
    const engineerPassword = await bcryptjs_1.default.hash("Engineer@123", 10);
    const viewerPassword = await bcryptjs_1.default.hash("Viewer@123", 10);
    const users = [
    ...
    for (const user of users) {
        await (0, db_1.query)("INSERT INTO users (name, email, password_hash, role) VALUES ($1,$2,$3,$4) ON CONFLICT (email) DO NOTHING", user);
    }
}
